...
async def ask_question(
    ...
    if query.new_chat:
        ...
        is_new_conversation = True
        print(f"🎯 NEW CHAT started: {conversation_id}")
    else:
        # Get active conversation or create one if none exists
        active_conversation = await conversation_service.get_active_conversation(current_user["_id"])
        if not active_conversation:
            ...
            is_new_conversation = True
            print(f"🆕 First conversation created: {conversation_id}")
        else:
            conversation_id = active_conversation["_id"]
    ...
    try:
        ...
        message_data = {
            ...
            "tokens_used": result.get("tokens_used"),
            "model_used": result["model_used"],
            "timestamp": datetime.now(timezone.utc)  # Make sure timestamp is set
        }
        ...
        # Convert to dict and ensure timestamp is included
        qa_record_dict = qa_record.model_dump()
        qa_record_dict["timestamp"] = datetime.now(timezone.utc)
        
        await qa_history_collection.insert_one(qa_record_dict)
