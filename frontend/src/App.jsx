...
const ChatPage = () => {
  ...
  const [currentMessages, setCurrentMessages] = useState([]);
  const [currentConversationId, setCurrentConversationId] = useState(null);
  const [isNewChat, setIsNewChat] = useState(false); // Track if we're starting a new chat

  // Handle new chat - Reset all states and prepare for new conversation
  const handleNewChat = () => {
    console.log('🆕 App: New chat requested');
    setSelectedHistory(null);
    setCurrentMessages([]);
    setCurrentConversationId(null);
    setIsNewChat(true); // Mark that next message should create new conversation
    setRefreshKey((prev) => prev + 1);
  };

  // Handle selecting a history item
  const handleSelectHistory = (historyItem) => {
    console.log('📁 App: Selecting conversation:', historyItem._id);
    setSelectedHistory(historyItem);
    setCurrentConversationId(historyItem._id);
    setCurrentMessages([]); // Clear current messages to load from selected conversation
    setIsNewChat(false); // Not a new chat when selecting existing conversation
    setRefreshKey((prev) => prev + 1);
  };

  // Handle new message sent
  const handleNewMessage = () => {
    console.log('💬 App: New message sent, refreshing sidebar');
    setIsNewChat(false); // After first message, we're no longer in "new chat" mode
    setRefreshKey((prev) => prev + 1);
  };
  ...
  // Handle conversation ID change
  const handleConversationIdChange = (conversationId) => {
    console.log('🆔 App: Conversation ID changed to:', conversationId);
    setCurrentConversationId(conversationId);
  };
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-gray-50">
      ...
      <ChatArea
        ...
        currentConversationId={currentConversationId}
        onConversationIdChange={handleConversationIdChange}
        isNewChat={isNewChat} // Pass this to ChatArea
      />
    </div>
  );
};
