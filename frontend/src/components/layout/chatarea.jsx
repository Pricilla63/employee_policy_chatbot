...
//   isNewChat // Add this prop from parent
// }) => {
//   const { user } = useAuth();
//   const [messages, setMessages] = useState([]);
//   const [input, setInput] = useState('');
//   const [loading, setLoading] = useState(false);
//   const [copiedIndex, setCopiedIndex] = useState(null);
//   const [isFirstMessageAfterNewChat, setIsFirstMessageAfterNewChat] = useState(false);
//   const messagesEndRef = useRef(null);

//   // Reset new chat flag when selectedHistory changes
//   useEffect(() => {
//     if (selectedHistory) {
//       setIsFirstMessageAfterNewChat(false);
//     }
//   }, [selectedHistory]);

//   // Set new chat flag when isNewChat prop changes
//   useEffect(() => {
//     if (isNewChat) {
//       console.log('💬 ChatArea: New chat mode activated');
//       setIsFirstMessageAfterNewChat(true);
//     }
//   }, [isNewChat]);
...
//     const loadConversation = async () => {
//       if (selectedHistory) {
//         console.log('💬 ChatArea: Loading conversation:', selectedHistory._id);
        
//         // If we have a conversation ID but no messages, fetch the full conversation
//         if (selectedHistory._id && (!selectedHistory.messages || selectedHistory.messages.length === 0)) {
//           try {
//             const response = await conversationsAPI.getConversation(selectedHistory._id);
//             console.log('💬 ChatArea: Fetched conversation:', response.data);
...
//             return;
//           } catch (error) {
//             console.error('❌ ChatArea: Error loading conversation:', error);
//           }
//         }
        
//         // Check if selectedHistory has messages array
//         if (selectedHistory.messages && Array.isArray(selectedHistory.messages) && selectedHistory.messages.length > 0) {
//           console.log('💬 ChatArea: Messages found:', selectedHistory.messages.length);
//           // Transform backend message format to ChatArea format
//           const formattedMessages = [];
//           selectedHistory.messages.forEach(msg => {
//             // Add user message
...
//         } else if (selectedHistory.question && selectedHistory.answer) {
//           // Fallback for old single Q&A format (backward compatibility)
//           console.log('💬 ChatArea: Using fallback format');
//           setMessages([
//             {
//               type: 'user',
//               content: selectedHistory.question,
...
//           ]);
//         } else {
//           console.warn('💬 ChatArea: No valid message format found in selectedHistory');
//           setMessages([]);
//         }
//       } else {
//         // New chat - start with empty messages or currentMessages
//         console.log('💬 ChatArea: Starting new chat, messages cleared');
//         setMessages(currentMessages || []);
//       }
//     };
...
//     try {
//       // Determine if this should create a new conversation
//       const shouldCreateNewChat = isFirstMessageAfterNewChat || (!selectedHistory && !currentConversationId);
      
//       console.log('💬 ChatArea: Sending message - new_chat:', shouldCreateNewChat);
      
//       const response = await queryAPI.askQuestion({
//         question: input,
//         new_chat: shouldCreateNewChat // THIS IS THE KEY FIX!
//       });
...
//       // Update conversation ID if returned
//       if (response.data.conversation_id && onConversationIdChange) {
//         console.log('💬 ChatArea: Conversation ID received:', response.data.conversation_id);
//         onConversationIdChange(response.data.conversation_id);
//       }
      
//       // Reset new chat flag after first message
//       if (isFirstMessageAfterNewChat) {
//         setIsFirstMessageAfterNewChat(false);
//       }
      
//       console.log('💬 ChatArea: Message sent successfully, calling onNewMessage');
//       onNewMessage?.(); // This will refresh the sidebar history
...
//       // Reset new chat flag even on error
//       if (isFirstMessageAfterNewChat) {
//         setIsFirstMessageAfterNewChat(false);
//       }
//     } finally {
...
//           Conversation: {currentConversationId ? currentConversationId.substring(0, 8) + '...' : 'New'} | 
//           New Chat Mode: {isFirstMessageAfterNewChat ? 'Yes' : 'No'}
//         </div>
//       </div>
...
//                 </h1>
//                 <p className="text-xl text-gray-600">How can I help you today?</p>
//                 {isFirstMessageAfterNewChat && (
//                   <p className="text-sm text-blue-600 mt-2">✨ Ready for a new conversation</p>
//                 )}
//               </div>
//             </div>
//           </div>
...
const ChatArea = ({ 
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // FIXED: Use askQuestion instead of ask, and send the correct data structure
      const response = await queryAPI.askQuestion({
        question: input,
        new_chat: shouldCreateNewChat // THIS IS THE KEY FIX!
      });
      
      console.log('💬 ChatArea: Response received:', response.data);
      ...
      if (response.data.is_new_conversation) {
        console.log('💬 ChatArea: New conversation created!');
      }
