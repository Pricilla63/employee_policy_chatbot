...
//   // Load history on mount and whenever refreshTrigger changes
//   useEffect(() => {
//     loadHistory();
//   }, [refreshTrigger]);
...
//     try {
//       // Use conversations API instead of history API
//       const response = await conversationsAPI.getConversations(50, 0);
...
//         await loadHistory();
//         // If we're currently viewing the deleted history, clear the selection
...
//   const handleNewChat = () => {
//     // Always trigger new chat and clear selection
//     onNewChat();
//   };
...
//       const itemDate = new Date(item.last_activity || item.updated_at || item.created_at);
...
//                 selectedHistory && selectedHistory._id === item._id
...
//                   {/* Use title or preview_message instead of question */}
//                   <p className="text-sm truncate pr-6 text-white">
//                     {item.title || item.preview_message || 'Conversation'}
//                   </p>
//                   {/* Show last activity time */}
//                   <p className="text-xs text-gray-400 mt-1">
//                     {formatDate(item.last_activity || item.updated_at || item.created_at)}
//                   </p>
//                 </div>
//                 <div
...
const Sidebar = ({ onNewChat, onSelectHistory, selectedHistory, refreshTrigger }) => {
  ...
  const loadHistory = async () => {
    try {
      ...
      const response = await conversationsAPI.getConversations(50, 0);
      console.log('✅ Sidebar: Conversations loaded:', response.data.length);
      setHistory(response.data);
    } catch (error) {
      console.error('❌ Sidebar: Error loading history:', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDeleteHistory = async (id, e) => {
    ...
    if (window.confirm('Delete this conversation?')) {
      try {
        ...
        // If we're currently viewing the deleted history, clear the selection
        if (selectedHistory && selectedHistory._id === id) {
          console.log('🆕 Sidebar: Triggering new chat after deletion');
          onNewChat();
        }
      } catch (error) {
  ...
  const handleSelectHistory = (item) => {
    console.log('📁 Sidebar: Selected conversation:', item._id, item.title);
    onSelectHistory(item);
  };
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {/* <p className="text-xs text-gray-500">
                    ID: {item._id?.substring(0, 8)}... | Messages: {item.message_count || 0}
                  </p> */}
                </div>
                <div
                  onClick={(e) => handleDeleteHistory(item._id, e)}
